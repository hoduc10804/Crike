...
declare type ActionBrief = Omit<Omit<Action, 'indices'>, 'name'>;
declare type ProtocolDetails = {
    name: string;
};
export declare type ParsedTranaction = {
    timestamp?: string;
    fee?: number;
    fee_payer: string;
    actions?: ActionBrief[];
...
declare type EasyProtocol = {
    address: string;
    name: string;
};
declare type SimplifiedTxnInfo = {
    timestamp: string;
    fee: number;
    fee_payer: string;
    signatures: string[];
...
declare type NeatActionSummary<T> = Omit<ActionSummary<T>, 'tags'>;
declare type Event = {
    name: string;
    data: any;
};
export declare type ParsedTxnSummary = SimplifiedTxnInfo & {
    actions: NeatActionSummary<any>[];
    events?: Array<Event>;
};
export declare type RawTransaction = ParsedTransactionWithMeta & {
    parsed: ParsedTxnSummary;
};
export declare type TransactionHistory = (ParsedTxnSummary & {
    raw?: ParsedTransactionWithMeta;
})[];
export declare type TxnCommitment = 'processed' | 'confirmed' | 'finalized';
declare type RpcError = {
    code: number;
    message: string;
    data: any;
};
export declare type SendTransactionResp = {
    id: number | string;
    signature: string;
    error?: RpcError;
